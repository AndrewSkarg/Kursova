<template>
  <div v-if="this.authorized">
      ...
      <li v-for="component in selectedComponents" :key="component.component_id">
        ...
        <template v-if="component.editing">
          <input
            ...
            min="0"
            max="1"
            v-model="component.DishComponent.countOfComp"
          />
        </template>
        <template
          v-else-if="
            component.DishComponent.countOfComp === '0' ||
            parseFloat(component.DishComponent.countOfComp) > 1
          "
        >
          <label
            >(Введіть вірну кількість! ✎)
            {{ (component.DishComponent.countOfComp = "0") }}/{{
              component.count
            }}
        ...
        <template v-else>
          <label
            >{{ component.DishComponent.countOfComp }}
            <span v-if="this.role === 'шеф'">/{{ component.count }} </span>
            {{ component.unit }}</label
          >
        </template>
    ...
    <div v-if="this.role === 'шеф'">
      ...
      <button class="styled-button" v-if="showAddButton" @click="toggleAddForm">
      ...
      <button class="styled-button" @click="saveChanges">Зберегти зміни</button>
    </div>
  </div>
  ...
  methods: {
    async saveChanges() {
      try {
        ...
        const modifiedArray = this.selectedComponents.map(({component_id,count,description,editing,priceForUnit,selected,title,unit,...selectedProps}) => {
            return selectedProps.DishComponent;
          }
        );
      ...
      this.components = valuesComp.map((component) => {
        return {
          ...
          DishComponent: {
            ...
            dishF: this.dishId,
            componentF: component.component_id,
            countOfComp: "0",
          },
        };
      });
    ...
    async addNewComponent(newComponent) {
      if (
        !this.selectedComponents.some(
          (comp) =>
            comp.title.toLowerCase() === newComponent.title.toLowerCase()
        ) &&
        !this.components.some(
          (comp) =>
            comp.title.toLowerCase() === newComponent.title.toLowerCase()
        ...
        await PostService.insertComponent(
          newComponent.title,
          newComponent.count,
          newComponent.priceForUnit,
          newComponent.unit,
